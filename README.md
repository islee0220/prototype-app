...
Our application's functionality will be revolving around the main dashboard. The dashboard will provide a daily task and event overview and quick access to course-related information, such as course syllabi. Users will be able to click on tasks to add detailed information, notes, or files, and assign different attributes to distinguish them by course, priority, or status. 
The second tab is a visual calendar will offer daily, weekly, and monthly views, allowing users to switch between tasks, events, or have both open. The application will use time blocks to visually represent events to be able to sense how the day is split up. Tasks with sooner deadlines can be automated to be labeled as high priority, while tasks due in the future are labeled with low priority. Larger, important projects can be manually labeled as high priority and further subdivided into sub-tasks, each with its dedicated page. 
